...
class Policy(nn.Module):
    def __init__(self,**policy_params):
        ...
        for key, value in policy_params.items():
            setattr(self, key, value)
        self.lstm = nn.LSTM(self.win_size, 32, 1)
    ...
    def forward(self, state):
        state = torch.reshape(state, (-1, 1, 3))#10,1,3
        lstm_out, _ = self.lstm(state)#10,1,32
        batch_n, win_s, hidden_s = lstm_out.shape
        lstm_out = lstm_out.view(batch_n, win_s * hidden_s)#10,32
        lstm_out = torch.reshape(lstm_out, (-1, self.product_num + 1, 32))#1,10,32
        lstm_out = lstm_out.view(lstm_out.size(0), -1)#1,320
        fc1_out = self.linear1(lstm_out)#1,64
        # fc1_out = F.relu(fc1_out)
        fc2_out = self.linear2(fc1_out)
        # fc2_out = F.relu(fc2_out)
        fc3_out = self.linear3(fc2_out)
    ...
    def train(self,env,env_test,loader = None):
        ...
        for episode in tqdm(range(self.episodes),desc=f'>>>Training with {self.device}, result saved in {self.result_path}<<<\n\t'):
            ...
            for train_step in tqdm(range(env.dataloader.max_step), desc=f'Training episode {episode}'):
                ...
                s1, r1, train_done, info = env.step(a0)
                ...
                if abs(r1-env.portfolio.entropy_loss)>=(np.mean(absreward_list)+np.std(absreward_list)):
            ...
            # self.policy_learn()
            # writer.add_scalar('Loss/Policy', self.policy_loss, episode)
            env.render()
